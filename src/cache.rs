...
pub async fn delete_cache_dir(cache_dir: &str, layers_config: LayersConfig) {
    for layer in layers_config.layers.iter() {
        if layer.delete_cache_on_start.unwrap() {
            let dir_path = Path::new(cache_dir).join(layer.name.to_string());
...
pub async fn get_cache(tilepath: PathBuf, max_cache_age: u64) -> Result<Bytes, anyhow::Error> {
...
pub async fn write_tile_to_file(tilepath: &PathBuf, tile: &[u8]) -> Result<(), anyhow::Error> {

...
feature-6 = Metadatos
feature-6-desc = Catálogo de metadatos basado en ISO 19115, accesible vía OGC API - Records.
...
toast-save-failed = No se pudo guardar.
toast-required-fields = Completá los campos obligatorios.
open-json = Abrir JSON
open-png = Abrir PNG
url-direct = URL directa
url-style-reference = Referencia para styles
url-ogc-records = OGC API - Records
url-openapi-scalar = OpenAPI (Scalar)
system = Sistema
sqlite-path = Ruta de la base de datos de configuración
...
topic-category-economy = Economía
topic-category-elevation = Elevación
topic-category-environment = Medio ambiente
topic-category-geoscientificInformation = Información geocientífica
topic-category-health = Salud
topic-category-imageryBaseMapsEarthCover = Imágenes, mapas base y cobertura terrestre
topic-category-intelligenceMilitary = Inteligencia y militar
topic-category-inlandWaters = Aguas continentales
topic-category-location = Ubicación
topic-category-oceans = Océanos
topic-category-planningCadastre = Planeamiento urbanístico y catastro
topic-category-society = Sociedad
topic-category-structure = Edificaciones
topic-category-transportation = Transporte
topic-category-utilitiesCommunication = Servicios públicos y comunicaciones
progress-code-completed = Completado
progress-code-historicalArchive = Archivo histórico
progress-code-obsolete = Obsoleto
progress-code-onGoing = En curso
progress-code-planned = Planificado
progress-code-required = Requerido
progress-code-underDevelopment = En desarrollo
role-code-originator = Originador
role-code-pointOfContact = Punto de contacto
role-code-metadataAuthor = Autor de los metadatos
role-code-custodian = Custodio
role-code-publisher = Editor
role-code-processor = Procesador
workflow-status-draft = Borrador
workflow-status-published = Publicado
metadata-with-record = Con registro
metadata-without-record = Sin registro
metadata-status = Estado
add-metadata = Agregar
confirm-delete-metadata = ¿Está seguro que quiere eliminar los metadatos de esta capa?
metadata-empty-title = No hay capas publicadas.
metadata-empty-subtitle = Publica una capa en el catálogo para poder agregarle metadatos.
